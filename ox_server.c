...
char account[TOT_ACCOUNT][10]={"hubert","ted","lily","peter"};
int account_len[TOT_ACCOUNT]={6,3,4,5};
...
void *pthread_service(void* sfd){
    ...
    while((recv_len=recv(fd,mes,BUFF_SIZE,0))>0){
        ...
        if (account_num==-1){   //not login yet
            ...
            if (!strncmp(mes,"!duel",5)){ //ask for a duel
                ...
                if (i<4){ //account_num is yourself
                    sprintf(mes,"!duel %s ask you to join the game!\n",account[account_num]);
            ...
            if (game_round>0 && agree[0]==1 && agree[1]==1){    //gaming
                if (game_round==1) printf("%s and %s start the game\n",account[room[0]],account[room[1]]);
                row=*(mes+8)-'0';
                col=*(mes+9)-'0';
                if (!strncmp(mes,"!master",7) && game_round%2){ //the one who start the game
                    if (game[row][col]==0){
                ...
                }else if (!strncmp(mes,"!player",7) && (game_round%2-1)){ //the one who join the game
                    if (game[row][col]==0){
